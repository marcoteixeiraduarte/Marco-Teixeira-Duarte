# Orte dieser Spur — Karte zuerst

Jedes `lugar` in einem Akt wird gegen **Karte und Pfarrei**
gehalten, bevor es feststeht. Google / OSM / Nominatim, CartTop,
dann die kirchliche Pfarrei (nicht nur Civil 2013).

Ein Name, der auf der Karte steht, wird nicht weggelogen, nur weil
ein Akt ein ähnliches Wort schreibt.

## Was wohin gehört

| Ort | Gibt es ihn? | Kirchliche Pfarrei | Civil heute | Beleg |
|---|---|---|---|---|
| **Bemposta** | ja, lugar | **Alvorge** (`PANS01`) | Alvorge, PLZ 3240-405 | OSM 39.976 N, 8.431 W; Akt 1874 `freguezia de Alvorge`; Auftraggeber |
| **Ladeia** | ja | **Alvorge** (Quinta); historisch die ganze Grenzlandschaft | Alvorge, PLZ 3240-402 (Ruinen) | OSM/SIPA/CM Ansião, siehe unten |
| **Lindos** | ja, **Weiler** in Torre de Vale de Todos | **Torre de Vale de Todos** (`PANS08`) | CartTop `R LINDOS` in TORRE DE VALE DE TODOS (035210) und VALE DE TODOS (017164) | Akten 1859/1860/1862/1864/1874 `lugar dos Lindos` |
| **Lindoso** | ja, **Norden** | eigene Freguesia Lindoso | Ponte da Barca, Distrikt Viana do Castelo | Wikipedia/OSM ~41.87 N, 8.20 W. Nicht diese Familie. |
| **Rua d'Além** | ja | Torre | Vale de Todos 017164 | CartTop `R ALEM` |
| **Barreira** | ja | Torre | Vale de Todos 017164 | CartTop `R BARREIRA`; Grundstücksakten |
| Stadt **Leiria** | ja, aber anderer Ort | — | Distrikthauptstadt | nicht Delfinas Lugar |

Kirchlich bis 2013: Bemposta und Ladeia in **Alvorge**; Lindos, Além
und Barreira in **Torre**. 2013 ging nur die Civil-Freguesia Torre
in Ansião auf. Die Taufbücher bleiben `PANS01` / `PANS08`.

## Ladeia

Es gibt sie. Nicht streichen.

Zwei Schichten, beide lokal:

1. **Quinta / Torre da Ladeia** (Ruinen, Fonte da Ladeia). OSM
   39.983 N, 8.451 W. SIPA: Portugal, Leiria, Ansião, **Alvorge**.
   CM Ansião: nördlich des Sitzes der Freguesia Alvorge, PLZ
   **3240-402**. Keine eigene Pfarrei.
2. **Landschaft Ladeia** — alte Bezeichnung für die bewegliche
   Grenze der Reconquista. Wikipedia/Freguesia-Text: Torre de Vale
   de Todos liegt am **Südsaum** dieser Ladeia. Die PR7 „Rota da
   Ladeia“ startet an der Junta Alvorge und läuft über Vale de Todos.

Wenn ein Akt `Ladeia` schreibt: zuerst **Alvorge-Bücher**. Nicht in
eine erfundene Pfarrei Ladeia. Nicht mit der Stadt Leiria tauschen.

Ladeia (Alvorge) und Lindos (Torre) sind Nachbarn in derselben
Landschaft, nicht derselbe Lugar.

## Bemposta

Gehört zu **Alvorge**. Das steht im Akt und auf der Karte.

OSM: Bemposta, Trás de Figueiró, Alvorge, 3240-405. Rund 1 km östlich
vom Dorf Alvorge; rund 1,5 km nördlich der Igreja Matriz Torre.
Nachbarweiler derselben Pfarrei: Trás de Figueiró, Vila Nova (Quinta
da Ladeia), Vale Florido, Ateanha, Aljazede.

Taufen: Alvorge (`PANS01`), oft Rand `Bemposta`.

Nicht: Bemposta (Penamacor).

## Lindos (Torre) und Lindoso (Norden)

Beide Namen gibt es. Es sind zwei Orte.

**Hier:** **Lindos** ist ein Weiler der Pfarrei **Torre de Vale de
Todos**. Die Akten schreiben `lugar dos Lindos, desta freguezia da
Torre` (Heirat 1859, Taufe João 1874). Die Jahresbände 1860/1862/1864
setzen an den Rand **`Lindos.`** CartTop katalogisiert denselben
Weiler heute als Straße `R LINDOS` — Locality **TORRE DE VALE DE
TODOS** (035210) und VALE DE TODOS (017164). OSM: Rua dos Lindos,
3240-710, an der Rua Nossa Senhora da Graça (Matriz).

**Im Norden:** **Lindoso**, eigene Freguesia im Kreis **Ponte da
Barca**, Distrikt Viana do Castelo (Alto Minho). Etwa 41.87 N,
8.20 W; Castelo de Lindoso, PLZ 4980-451. Rund 200 km nördlich von
Ansião. Eigene Pfarrei, andere Diözese. Nicht in den Ansião-Büchern.

Nominatim `Lindoso Ansião` ist leer — in diesem Kreis gibt es nur
**Lindos**. Die frühere Lesung `lugar da Leiria` war die Stadt;
die Lesung `Lindoso` war der Ort im Norden. Beides nicht Delfina.

CartTop 2019, dieser Kreis (nicht Ponte da Barca):

| Form | ID | Lugar | Locality |
|---|---|---|---|
| `R LINDOS` | `22110031016603` | VALE DE TODOS | 017164 |
| `R LINDOS` | `22110031016604` | TORRE DE VALE DE TODOS | 035210 |
| `R ALEM` | `22110031016263` | VALE DE TODOS | 017164 |
| `R BARREIRA` | `22110031016286` | VALE DE TODOS | 017164 |

OSM: Rua dos Lindos, Torre de Vale de Todos, 3240-710. Sie trifft
auf die Rua Nossa Senhora da Graça (Matriz). Delfina ist natural
am Kirchenort, Luiz kommt aus Bemposta/Alvorge über die Pfarreigrenze.

## Distanz, die den Akt erklärt

Bemposta (Alvorge) → Igreja Torre ≈ 2 km. Luiz natural Bemposta,
Delfina natural Lindos, Heirat in Torre, 1859–1867 wohnhaft Lindos,
1874 Rua d'Além, später Barreira: alles dieselbe Pfarrei Torre nach
der Heirat, plus die Herkunftspfarrerei Alvorge für Luiz.
